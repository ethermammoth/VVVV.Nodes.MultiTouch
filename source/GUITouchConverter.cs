...
namespace VVVV.Nodes
{
    ...
    #region PluginInfo
    [PluginInfo(
        Name = "iOS Touch Converter", 
        Category = "GUI", 
        Help = "Simple MultiTouch Filter", 
        Tags = "multitouch",
        Author = "ethermammoth"
    )]
    #endregion PluginInfo
    public class GUITouchConverter : IPluginEvaluate
    {
        #region fields & pins
        ...
        [Input("Time Threshold", DefaultValue = 1000, IsSingle = true)]
        public ISpread<int> FTimeThreshold;

        [Input("Enable Movement Threshold", DefaultBoolean=true, IsSingle = true)]
        ...
        [Output("Touch Lifetime")]
        public ISpread<int> FTouchLifetimeOut;
            ...
            if (FMoveEnabled[0])
            {
                for (int x = 0; x < FTouchId.SliceCount; x++)
                {
                    if (candidateList.ContainsKey(FTouchId[x]))
                    {
                        ...
                        if (running)
                            candidateList[FTouchId[x]].lifeTime = (int)ftimer.ElapsedMilliseconds - candidateList[FTouchId[x]].startTime;
                        if (candidateList[FTouchId[x]].status == 1)
                        {
                            ...
                            if (running)
                            {
                                if (candidateList[FTouchId[x]].lifeTime > FTimeThreshold[0])
                                {
                                    candidateList[FTouchId[x]].status = 0;
                                }
                            }
                ...
                FTouchLifetimeOut.SliceCount = candidateList.Count;
                ...
                foreach (KeyValuePair<int, TouchPoint> entry in candidateList)
                {
                    ...
                    FTouchLifetimeOut[m] = entry.Value.lifeTime;
